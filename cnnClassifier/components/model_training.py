...
class Training:
    ...
    def train_valid_generator(self):
        ...
        if self.config.params_is_augmentation:
            ...
            rotation_range=30,  # 🔹 Reduced from 45 to 30 (less distortion)
            horizontal_flip=True,
            vertical_flip=False,  # ❌ Disabled vertical flip (not common in medical imaging)
            width_shift_range=0.2,  # 🔹 Reduced from 0.3 to 0.2
            height_shift_range=0.2,  # 🔹 Reduced from 0.3 to 0.2
            shear_range=0.2,  # 🔹 Reduced from 0.3 to 0.2
            zoom_range=0.2,  # 🔹 Reduced from 0.3 to 0.2
            brightness_range=[0.9, 1.1],  # 🔹 Adjust brightness slightly (was 0.8-1.2)
            rescale=1./255,
            validation_split=0.20
        )
        else:
    ...
    def train(self):
        ...
        try:
            self.model.fit(
                ...
                class_weight=class_weight_dict,  # ✅ Helps balance classes
